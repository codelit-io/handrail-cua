...
| R-3.4-C | Classified values stay out of artifacts, model requests, and unsafe evidence; exact per-run request provenance remains reviewable without retaining prompts | `src/model/planner.ts`, `src/runtime/discovery.ts`, `src/runtime/redaction.ts`, `src/runtime/evidence.ts` | `test/planner.test.ts`, `test/discovery.test.ts`, `test/redaction.test.ts` | prompt-trace hash, evidence sensitive-pattern scan, and screenshot review |
