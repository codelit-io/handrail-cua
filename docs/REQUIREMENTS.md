...
Source: Interface.ai Computer-Use Automation System assignment, Sections 1-11; Section 10 is glossary context
...
| R-3.1-C | Step, active-automation time, recovery, stale-ref, invalid-decision, and dead-end bounds stop safely; runtime-owned decision IDs bind every accepted decision to one fresh observation | `src/runtime/discovery.ts`, `src/runtime/replay.ts` | `test/discovery.test.ts`, `test/replay.test.ts` | projected fault/intervention events and unique decision/observation bindings |
...
| R-3.2-C | Bind review to immutable content | artifact digest, approval chronology, and strict artifact approval in `src/runtime/artifact.ts`, `src/runtime/replay.ts`, and the externally gated live workflow in `src/cli.ts` | `test/artifact.test.ts`, `test/cli.test.ts`, `test/replay.test.ts` | manifest-bound artifact hash and separately issued approval record |
...
| R-3.4-A | Exact origin, route, command, effect, and supported browser transport must pass every policy layer immediately before action | `src/runtime/policy.ts`, `src/surface/browser-surface.ts`, discovery/replay policy calls, operator authorizer in `src/cli.ts` | `test/policy.test.ts`, `test/browser-surface.test.ts`, `test/discovery.test.ts`, `test/operator.test.ts`, `test/replay.test.ts` | automated policy/transport-denial proof and a structured fault/event when that scenario is retained |
...
| R-3.4-C | Classified values stay out of artifacts, model requests, and unsafe evidence; exact per-run request provenance remains reviewable without retaining prompts | `src/model/planner.ts`, `src/runtime/discovery.ts`, `src/runtime/artifact.ts`, `src/runtime/redaction.ts`, `src/runtime/evidence.ts`, `scripts/validate-evidence.ts` | `test/artifact.test.ts`, `test/planner.test.ts`, `test/discovery.test.ts`, `test/redaction.test.ts`, `test/evidence-validator.test.ts` | prompt-trace hash, type-aware evidence sensitive-pattern scan, and screenshot review |
| R-3.5-A | Persist one ordered, correlated audit-event contract with runtime-owned decision identities | `PersistedAuditEventSchema` and `EvidenceWriter` in `src/runtime/evidence.ts`; discovery decision-ID assignment in `src/runtime/discovery.ts` | `test/redaction.test.ts`, `test/discovery.test.ts`, `test/evidence-validator.test.ts` | every manifest event log validates sequence, IDs, observation bindings, run identity, and terminal event |
...
| R-3.6-D | Operator access and actions are capability-gated, policy-authorized, redacted, and durable | `src/operator/server.ts`, `src/cli.ts`, `src/runtime/evidence.ts` | `test/operator.test.ts`, `test/e2e.test.ts`, `test/evidence-validator.test.ts` | durable `operator.audit` events and three capture records, including byte-distinct states around the authorized recovery click |
...
| R-3.7-B | Separate vendor capability from tenant binding and fail closed on drift or an unreviewed replay override; discovery forbids overrides | `AppBindingSchema`, reviewed override binding, discovery preflight, fingerprint preflight | `test/artifact.test.ts`, `test/discovery.test.ts`, `test/replay.test.ts` | checked artifact/binding explanation in SDD |
...
| R-11-A | Publish source as a public repository and provide the repository URL for submission; sending email remains an external action | release workflow and root `CHECKLIST.md` | public-visibility, CI, and anonymous-clone verification | public `codelit-io/handrail-cua` release; submission copy is prepared but not sent without separate authorization |
...
5. The local approval record represents a trusted catalog decision. Qualifying live generation persists discovery and then waits for a separately issued, chronologically valid approval before replay; it never self-approves. Artifact approval remains separate from operation-bound risky-action approval, which is consumed once within a replay invocation, and from a short-lived human ownership grant. Global run-ID uniqueness and cross-process nonce consumption are control-plane responsibilities outside this local demo.
...
7. The approval reviewer label `independent-process-review` means the approval was issued through a separate local command after inspecting the generated artifact. It does not claim an independent company, third-party auditor, or external human attestation.
...
| 4. Handoff | one live surface, exclusive epoch ownership, policy on every operator action | capability-gated operator console, epoch claim, durable intent/completion audit in evaluator entrypoints, fresh checkpoint resume | same-session IDs, monotonic epochs, captures, integrated e2e |
| 5. Hardening | independently challenge data egress, approval scope, override integrity, evidence claims, and loopback access | fail-closed defaults, consumed operation-bound authority, digest-reviewed replay-only override binding, strict manifest v1.2 | security/QA review, negative tests, dependency and secret scans |
...
- The fragment capability is a bearer credential that remains reusable for the intervention lifetime even after a browser exchanges it for a path-scoped cookie; it is not user identity and must not be logged, persisted, or shared beyond the assigned operator.
- Durable operator auditing is guaranteed by the evaluator CLI wiring, which supplies audit and capture sinks. The embeddable HTTP server can run with in-memory audit only if a caller omits those sinks; production integration must make durable sinks and workforce identity mandatory.
